...
桌面支持段落对齐和缩进、带样式及原位编辑的网格、可缩放的行内和边缘图像、图片超链接，以及 MOD/XM/S3M/IT 音乐和采样音频。支持 Inform 加速函数 1–13。系统字体提供 Unicode 回退；Settings 中可分别为比例正文和等宽文本调用 Windows/macOS 或 GTK 3 桌面的原生字体选择器，也可填写一个 TTF/OTF/TTC 回退字体文件。所选字体只安装到指定的比例／等宽类别；等宽和 HW 字体在比例正文中仅作为缺字回退，不会覆盖比例拉丁字形。已测试的媒体格式及限制见兼容性说明。
