...
Debug builds start a loopback HTTP profiling endpoint by default; release builds read the optional `profiling` object from `glulx-settings.json`. Its `enabled` and `sampling` switches control the listener and platform ETW marker provider, and its `address` may be non-loopback, but such listeners require `token`, sent as `Authorization: Bearer TOKEN`; loopback listeners may remain unauthenticated. Changes take effect after restarting the player. The endpoint exposes VM snapshots, opcode counters, stage timings and decode-cache statistics. Linux and macOS also expose pprof-rs protobuf and flamegraph reports. Windows registers a PID-scoped ETW provider and can request a bounded process-filtered WPR ETL through `/debug/etw/profile?seconds=N`; the generated provider GUID is derived from the target PID, with `ProcessExeFilter` retained as defense in depth, and captures marker call stacks without enabling kernel-wide `SampledProfile` CPU sampling. A missing tracing privilege triggers a UAC-elevated, short-lived helper that receives the target PID. JSON observability remains available because pprof-rs's POSIX sampler is unavailable there.
...
and settings; only the root canvas determines Glk dimensions. Graphics retain clipped image/rectangle primitives, published with shared window
