...
更新：2026-09-10。当前实现状态为 `bad16fb`。`[x]` 表示实现及列出的验证已完成，不表示穷尽规范认证；开放工作使用 `[ ]`。规范依据、类别和验收条件见 [剩余能力复核](glulx-remaining-spec-audit.ZH.md)。详细测试、样本版本、复现命令见 [验收记录](glulx-validation.ZH.md)，实现限制见 [兼容性](compatibility.ZH.md)。
...
- [x] 272 个库测试和 6 个 CLI 测试通过；另有 6 个性能测试保留为手动测量。按领域的测试矩阵和边界覆盖见验收记录。
